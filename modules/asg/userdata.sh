...
echo $x
sudo mysql -h "$x" -P 3306 -u ${master_user} -p${master_pass} -e "CREATE DATABASE IF NOT EXISTS drupal;"
echo $x
sudo mysql -h "$x" -P 3306 -u ${master_user} -p${master_pass} -e "CREATE USER IF NOT EXISTS '${sql_user}'@'%' IDENTIFIED BY '${sql_pass}'; GRANT ALL  ON drupal.* TO ${sql_user}@'%' IDENTIFIED BY '${sql_pass}' WITH GRANT OPTION; FLUSH PRIVILEGES;"
echo $x
...
sudo ./vendor/bin/drush site-install standard --db-url=mysql://${sql_user}:${sql_pass}@"$x"/drupal --site-name=Example --account-name=${drupal_user} --account-pass=${drupal_pass} --yes
