...
resource "aws_key_pair" "generated_key" {
  key_name   = "${terraform.workspace}-monitoring-key"
  public_key = tls_private_key.dev_key.public_key_openssh

  provisioner "local-exec" {    
    command = "echo '${tls_private_key.dev_key.private_key_pem}' > ./${terraform.workspace}-monitoring-key.pem"
  }

  provisioner "local-exec" {
    command = "chmod 400 ./${terraform.workspace}-monitoring-key.pem"
  }
}
...
resource "aws_instance" "terraform-test-2" {
  ...
  tags = {
		Name = "${terraform.workspace}-Drupal-Monitor"
	}
}
